...
def rollout_policy_fn(board):
    ...
    # rollout randomly
    action_probs = np.random.rand(len(board.avaliableMove()))
    return zip(board.avaliableMove(), action_probs)    

def policy_value_fn(board):
    """a function that takes in a state and outputs a list of (action, probability)
    tuples and a score for the state"""
    # return uniform probabilities and 0 score for pure MCTS
    action_probs = np.ones(len(board.avaliableMove()))/len(board.avaliableMove())
    return zip(board.avaliableMove(), action_probs), 0
...
class MCTS(object):
    ...
    def _playout(self, state):
        ...
        leaf_value = self._evaluate_rollout(result, state)
        # print(leaf_value)
        # Update value and visit count of nodes in this traversal.
        node.update_recursive(-leaf_value)

    def _evaluate_rollout(self, result, state, limit=1000):
        ...
        for i in range(limit):
            ...
            result = state.doMove(max_action)
        else:
            # If no break from the loop, issue a warning.
            print("WARNING: rollout reached move limit")
        if result == 1 and state.curStepColor == SHOGIMAN_COLOR_DOWN:  
            return 1.0
        elif result == 2 and state.curStepColor == SHOGIMAN_COLOR_UP:
            return 1.0
        elif result == 1 and state.curStepColor == SHOGIMAN_COLOR_UP:
            return -1.0
        elif result == 2 and state.curStepColor == SHOGIMAN_COLOR_DOWN:
            return -1.0
        else: # tie
            return 0.0
...
class MCTSPlayer(object):
    ...
    def get_action(self, board):
        ...
        if len(sensible_moves) > 0:
            ...
            print("WARNING: no available move")
