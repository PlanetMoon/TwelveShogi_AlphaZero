...
ps: Modified based on mcts_alphaZero.py from Junxiao Song' AlphoZero_Gomoku
@author: Xingye Xu
"""
...
class MCTS(object):
    ...
    def _playout(self, state):
        ...
        while(1):            
            ...
            result = state.doMove(action)
            i = i + 1
            if result == -1:
                print("mcts-MCTS._playout: level = %d, illegal action = %d" % (i, action))
        ...
        if result == 0:
            ...
            if result == 2 and state.curStepColor == SHOGIMAN_COLOR_DOWN:  
                leaf_value = 1.0
            elif result == 1 and state.curStepColor == SHOGIMAN_COLOR_UP:
                leaf_value = 1.0
            elif result == 2 and state.curStepColor == SHOGIMAN_COLOR_UP:
                leaf_value = -1.0
            elif result == 1 and state.curStepColor == SHOGIMAN_COLOR_DOWN:
                leaf_value = -1.0
            else: # tie
                leaf_value = 0.0
...
class MCTSPlayer(object):
    ...
    def __init__(self, policy_value_function, c_puct=5, n_playout=2000, is_selfplay=0):
        # policy_param = pickle.load(open('best_policy_6_6_4.model', 'rb'), encoding="bytes")
        # best_policy = PolicyValueNet(net_params = policy_param)
        # best_policy = PolicyValueNet()
        self.mcts = MCTS(policy_value_function, c_puct, n_playout)
        self._is_selfplay = is_selfplay
    
    def set_player_ind(self, color):
        self.color = color
    ...
    def get_action(self, board, temp=1e-3, return_prob=0):
        sensible_moves = board.avaliableMove()
        move_probs = np.zeros(6*6*12) # the pi vector returned by MCTS as in the alphaGo Zero paper
