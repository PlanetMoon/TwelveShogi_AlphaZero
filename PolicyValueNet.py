...
class PolicyValueNet():
    ...
    def create_policy_value_net(self):
        ...
        network = lasagne.layers.InputLayer(shape=(None, 34, self.board_width, self.board_height),
        ...
        self.policy_net = lasagne.layers.DenseLayer(policy_net, num_units=self.board_width*self.board_height*12, 
                                            nonlinearity=lasagne.nonlinearities.softmax)
        # state value layers
        value_net = lasagne.layers.Conv2DLayer(network, num_filters=2, filter_size=(1, 1))
        value_net = lasagne.layers.DenseLayer(value_net, num_units=self.board_width*self.board_height*12)
    ...
    def policy_value_fn(self, board):
        ...
        """
        legalMoves = board.avaliableMove()
        current_state = board.currentState()
        act_probs, value = self.policy_value(current_state.reshape(-1, 34, self.board_width, self.board_height))
        act_probs = zip(legalMoves, act_probs.flatten()[legalMoves])
